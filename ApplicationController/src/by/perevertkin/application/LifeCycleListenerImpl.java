package by.perevertkin.application;
...
public class LifeCycleListenerImpl implements LifeCycleListener {
    ...
    public void start() {
        try {
            Utility.ApplicationLogger.severe("start---->");
    ...
    private static void initializeDatabaseFromScript() throws Exception {
        Utility.ApplicationLogger.severe("inintialize---->");
        InputStream scriptStream = null;
        Connection conn = null;
        try {
            String docRoot = AdfmfJavaUtilities.getDirectoryPathRoot(AdfmfJavaUtilities.ApplicationDirectory);
            String dbName = docRoot + "/dbapp.db";

            File dbFile = new File(dbName);
            if (dbFile.exists())
                return;

            conn = new SQLite.JDBCDataSource("jdbc:sqlite:" + docRoot + "/dbapp.db").getConnection();
            conn.setAutoCommit(false);
            Utility.ApplicationLogger.severe("conn---->"+conn.toString());
            scriptStream =
                Thread.currentThread().getContextClassLoader().getResourceAsStream("META-INF/initial.sql");
            ...
            while ((nextLine = scriptReader.readLine()) != null) {
                ...
                if (nextLine.endsWith(";")) {
                    ...
                    Utility.ApplicationLogger.severe("statement---->"+nextStatement.toString());
